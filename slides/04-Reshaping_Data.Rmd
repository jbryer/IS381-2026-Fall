...
---
class: font80
# Value Matching (`a %in% b` and `a %notin% b`)

```{r, include=FALSE, results='hide'}
options(width = 120)
```

What if we want to know the position of the vowels within the alphabet. To start, we can use the built-in `letters` vector and create a vector for the vowels:
...
The `%in%` operator will look for each element on the left-hand-side within the right-hand-side. The result is a logical vector equal to the length of the left-had-side vector.
...
We can use the `which()` function to find the positions within the logical vector that are `TRUE`.
